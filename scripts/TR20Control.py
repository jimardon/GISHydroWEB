...
Main_increment = float(GetParameterAsText(set_i)); set_i = set_i + 1
detail = GetParameterAsText(set_i) == 'true'; set_i = set_i + 1
ARC = GetParameterAsText(set_i); set_i = set_i + 1
ARF = GetParameterAsText(set_i) == 'true'; set_i = set_i + 1
DelMarVa = GetParameterAsText(set_i); set_i = set_i + 1
cb_list = json.loads(GetParameterAsText(set_i)); set_i = set_i + 1
prec_uSpecified = json.loads(GetParameterAsText(set_i)); set_i = set_i + 1
areami2 = float(GetParameterAsText(set_i)); set_i = set_i + 1
ratingtype = json.loads(GetParameterAsText(set_i)); set_i = set_i + 1
minstage = json.loads(GetParameterAsText(set_i)); set_i = set_i + 1
reachno = json.loads(GetParameterAsText(set_i)); set_i = set_i + 1
rating = json.loads(GetParameterAsText(set_i)); set_i = set_i + 1
userda = GetParameterAsText(set_i); set_i = set_i + 1
usercn = GetParameterAsText(set_i); set_i = set_i + 1
usertc = GetParameterAsText(set_i); set_i = set_i + 1
...
for i, p in zip(cb_list, prec_uSpecified):
    ...
    stormname = "p%s-%s" % (year[-1], critdur[0:2])
    if upper90:
        stormname = "p%s-%su" % (year[-1], critdur[0:2])
...
for theyear, thecritdur, p, s in zip(year, stormdur, RF_precip, storm_response[0]):
    ...
    if s == "Type":
        ...
        stormstring = stormstring + "{:<10}{:<10}{:<20}{:<10}{:<10}".format("", "p" + theyear +"-" + thecritdur, "GAGE", precavg, "rtp" +theyear + "-" + thecritdur + upper90str) + ARC + "\n"
...
SetParameterAsText(set_i, inputstring); set_i = set_i + 1
SetParameterAsText(set_i, outputstring); set_i = set_i + 1
SetParameterAsText(set_i, errorstring); set_i = set_i + 1