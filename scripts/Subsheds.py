...
"""
if uploadedlayer:    
    CopyFeatures_management(os.path.join(optfolder, "wshed.shp"), subshed)
    AddField_management(subshed, "ARCID", "DOUBLE", 10, "")
    AddField_management(subshed, "GRIDCODE", "DOUBLE", 10, "")
    
    with UpdateCursorda(subshed, ["ARCID","GRIDCODE"]) as cursor:
        for row in cursor:
            row[0] = 1
            row[1] = 1
